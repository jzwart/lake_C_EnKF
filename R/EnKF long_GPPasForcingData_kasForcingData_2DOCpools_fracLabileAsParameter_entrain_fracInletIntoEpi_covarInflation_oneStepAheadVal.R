...
Y_val <- Y # storing simulations of states / params before update for validation 
...
for(t in 2:nStep){
  for(i in 1:nEn){
    ...
    Y_val[1:5,1,t,i]<-Y[1:5,1,t-1,i]
    Y_val[6:9,1,t,i]<-z$X[1:4,1,t,i]
...
for(i in 1:nEn){
  ...
  lines(Y_val[9,1,,i]/data2$epiVol*12,col='blue',ylab='')
}
...
print(sqrt(mean((apply(Y_val[6,1,,],MARGIN = 1,FUN=mean)/data2$epiVol*12-z$y[1,1,,1]/data2$epiVol*12)^2,na.rm=T)))
print(sqrt(mean((apply(Y_val[9,1,,],MARGIN = 1,FUN=mean)/data2$epiVol*12-z$y[2,1,,1]/data2$epiVol*12)^2,na.rm=T)))

cor(apply(Y_val[6,1,!is.na(z$y[1,1,,1]),],MARGIN = 1,FUN=mean)/data2$epiVol[!is.na(z$y[1,1,,1])]*12,z$y[1,1,!is.na(z$y[1,1,,1]),1]/data2$epiVol[!is.na(z$y[1,1,,1])]*12)^2
cor(apply(Y_val[9,1,!is.na(z$y[2,1,,1]),],MARGIN = 1,FUN=mean)/data2$epiVol[!is.na(z$y[2,1,,1])]*12,z$y[2,1,!is.na(z$y[2,1,,1]),1]/data2$epiVol[!is.na(z$y[2,1,,1])]*12)^2

#bias 
mean(apply(Y_val[6,1,,],MARGIN = 1,FUN=mean)/data2$epiVol*12-z$y[1,1,,1]/data2$epiVol*12,na.rm = T)^2
mean(apply(Y_val[9,1,,],MARGIN = 1,FUN=mean)/data2$epiVol*12-z$y[2,1,,1]/data2$epiVol*12,na.rm=T)^2

# # mean RMSE for states; mol C 
print(sqrt(mean((apply(Y_val[6,1,,],MARGIN = 1,FUN=mean)-z$y[1,1,,1])^2,na.rm=T)))
print(sqrt(mean((apply(Y_val[9,1,,],MARGIN = 1,FUN=mean)-z$y[2,1,,1])^2,na.rm=T)))

# r2; mol C 
cor(apply(Y_val[6,1,!is.na(z$y[1,1,,1]),],MARGIN = 1,FUN=mean),z$y[1,1,!is.na(z$y[1,1,,1]),1])^2
cor(apply(Y_val[9,1,!is.na(z$y[2,1,,1]),],MARGIN = 1,FUN=mean),z$y[2,1,!is.na(z$y[2,1,,1]),1])^2
