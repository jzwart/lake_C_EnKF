...
hist(fracInVec)
  ...
  for(t in 2:nStep){
    for(i in 1:nEn){
      ...
      Y_val[1:3,1,t,i]<-Y[1:3,1,t-1,i] #r20,same as previous timestep
      Y_val[4:5,1,t,i]<-z$X[1:2,1,t,i] #forecasted states 
  ...
  for(i in 1:nEn){
    ...
    lines(Y_val[5,1,,i]/data2$epiVol*12,col='blue',ylab='')
  }
  ...
  print(sqrt(mean((apply(Y_val[4,1,,],MARGIN = 1,FUN=mean)/data2$epiVol*12-z$y[1,1,,1]/data2$epiVol*12)^2,na.rm=T)))
  print(sqrt(mean((apply(Y_val[5,1,,],MARGIN = 1,FUN=mean)/data2$epiVol*12-z$y[2,1,,1]/data2$epiVol*12)^2,na.rm=T)))
  
  cor(apply(Y_val[4,1,!is.na(z$y[1,1,,1]),],MARGIN = 1,FUN=mean)/data2$epiVol[!is.na(z$y[1,1,,1])]*12,z$y[1,1,!is.na(z$y[1,1,,1]),1]/data2$epiVol[!is.na(z$y[1,1,,1])]*12)^2
  cor(apply(Y_val[5,1,!is.na(z$y[2,1,,1]),],MARGIN = 1,FUN=mean)/data2$epiVol[!is.na(z$y[2,1,,1])]*12,z$y[2,1,!is.na(z$y[2,1,,1]),1]/data2$epiVol[!is.na(z$y[2,1,,1])]*12)^2
  
  #bias 
  mean(apply(Y_val[4,1,,],MARGIN = 1,FUN=mean)/data2$epiVol*12-z$y[1,1,,1]/data2$epiVol*12,na.rm = T)^2
  mean(apply(Y_val[5,1,,],MARGIN = 1,FUN=mean)/data2$epiVol*12-z$y[2,1,,1]/data2$epiVol*12,na.rm=T)^2
  
  # # mean RMSE for states; mol C 
  print(sqrt(mean((apply(Y_val[4,1,,],MARGIN = 1,FUN=mean)-z$y[1,1,,1])^2,na.rm=T)))
  print(sqrt(mean((apply(Y_val[5,1,,],MARGIN = 1,FUN=mean)-z$y[2,1,,1])^2,na.rm=T)))
  
  # r2; mol C 
  cor(apply(Y_val[4,1,!is.na(z$y[1,1,,1]),],MARGIN = 1,FUN=mean),z$y[1,1,!is.na(z$y[1,1,,1]),1])^2
  cor(apply(Y_val[5,1,!is.na(z$y[2,1,,1]),],MARGIN = 1,FUN=mean),z$y[2,1,!is.na(z$y[2,1,,1]),1])^2
