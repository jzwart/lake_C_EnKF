...
# setting up initial parameter values for all ensemble members from 2014 fits 
rVec<-matrix(Y[1,1,length(Y[1,1,,1]),]) # each row is an ensemble member 
rVec_fast<-matrix(Y[2,1,length(Y[1,1,,1]),])
fracVec<-matrix(Y[3,1,length(Y[1,1,,1]),])
fracInVec<-matrix(Y[4,1,length(Y[1,1,,1]),])
covar_inflat_vec <- matrix(Y[5,1,length(Y[1,1,,1]),])
...
# axis(4)
# 
# # RMSE for states ; only for evaluation years 
windows()
...
print(sqrt(mean((apply(Y[6,1,data2$timeStep>nStep14,],MARGIN = 1,FUN=mean)/data2$epiVol[data2$timeStep>nStep14]*12-z$y[1,1,data2$timeStep>nStep14,1]/data2$epiVol[data2$timeStep>nStep14]*12)^2,na.rm=T)))
print(sqrt(mean((apply(Y[9,1,data2$timeStep>nStep14,],MARGIN = 1,FUN=mean)/data2$epiVol[data2$timeStep>nStep14]*12-z$y[2,1,data2$timeStep>nStep14,1]/data2$epiVol[data2$timeStep>nStep14]*12)^2,na.rm=T)))

cor(apply(Y[6,1,!is.na(z$y[1,1,,1])&data2$timeStep>nStep14,],MARGIN = 1,FUN=mean)/data2$epiVol[!is.na(z$y[1,1,,1])&data2$timeStep>nStep14]*12,
    z$y[1,1,!is.na(z$y[1,1,,1])&data2$timeStep>nStep14,1]/data2$epiVol[!is.na(z$y[1,1,,1])&data2$timeStep>nStep14]*12)^2
cor(apply(Y[9,1,!is.na(z$y[2,1,,1])&data2$timeStep>nStep14,],MARGIN = 1,FUN=mean)/data2$epiVol[!is.na(z$y[2,1,,1])&data2$timeStep>nStep14]*12,
    z$y[2,1,!is.na(z$y[2,1,,1])&data2$timeStep>nStep14,1]/data2$epiVol[!is.na(z$y[2,1,,1])&data2$timeStep>nStep14]*12)^2

#bias 
mean(apply(Y[6,1,data2$timeStep>nStep14,],MARGIN = 1,FUN=mean)/data2$epiVol[data2$timeStep>nStep14]*12-z$y[1,1,data2$timeStep>nStep14,1]/data2$epiVol[data2$timeStep>nStep14]*12,na.rm = T)^2
mean(apply(Y[9,1,data2$timeStep>nStep14,],MARGIN = 1,FUN=mean)/data2$epiVol[data2$timeStep>nStep14]*12-z$y[2,1,data2$timeStep>nStep14,1]/data2$epiVol[data2$timeStep>nStep14]*12,na.rm=T)^2

# # mean RMSE for states; mol C 
print(sqrt(mean((apply(Y[6,1,data2$timeStep>nStep14,],MARGIN = 1,FUN=mean)-z$y[1,1,data2$timeStep>nStep14,1])^2,na.rm=T)))
print(sqrt(mean((apply(Y[9,1,data2$timeStep>nStep14,],MARGIN = 1,FUN=mean)-z$y[2,1,data2$timeStep>nStep14,1])^2,na.rm=T)))

# r2; mol C 
cor(apply(Y[6,1,!is.na(z$y[1,1,,1])&data2$timeStep>nStep14,],MARGIN = 1,FUN=mean),z$y[1,1,!is.na(z$y[1,1,,1])&data2$timeStep>nStep14,1])^2
cor(apply(Y[9,1,!is.na(z$y[2,1,,1])&data2$timeStep>nStep14,],MARGIN = 1,FUN=mean),z$y[2,1,!is.na(z$y[2,1,,1])&data2$timeStep>nStep14,1])^2
