...
for(i in 1:nEn){
  B[,,1,i]<-matrix(c(1-data2$QoutInt[1]/data2$epiVol[1]-data2$kCO2[1]/data2$thermo.depth[1],
                rFunc(rVec[i],data2$wtr[1]),0,1-rFunc(rVec[i],data2$wtr[1])-data2$QoutInt[1]/data2$epiVol[1]),nrow=2,byrow=T)
}
...
C<-array(c(rep(NA,nEn),rep(NA,nEn),rep(NA,nEn),rep(NA,nEn),rep(NA,nEn)),dim=c(2,5,nStep,nEn)) # array of transition matrix C[a,b,c,d]; 
#intializing first time step 
for(i in 1:nEn){
  C[,,1,i]<-matrix(c(data2$kCO2[1],1,-data2$GPP[1],0,0,0,0,0,data2$GPP[1],1),nrow=2,byrow=T)
}

ut<-array(c(rep(NA,nEn),rep(NA,nEn),rep(NA,nEn),rep(NA,nEn),rep(NA,nEn)),dim=c(5,1,nStep,nEn)) # array of transition matrix C[a,b,c,d]; 
#intializing first time step 
for(i in 1:nEn){
  ut[,,1,i]<-matrix(c(data2$DICeq[1]*data2$epiVol[1]/data2$thermo.depth[1],
                      data2$dicIn[1],(1-GPPrespired),
                      exudeTotal,data2$docIn[1]),ncol=1)
}
...
for(t in 2:nStep){
  for(i in 1:nEn){
    ...
    z$B[,,t,i]<-matrix(c(1-data2$QoutInt[t]/data2$epiVol[t]-data2$kCO2[t]/data2$thermo.depth[t], # zmix is fraction of pool available for exchange with atm 
                       rFunc(z$pars[1,1,t,i],data2$wtr[t]),0,1-rFunc(z$pars[1,1,t,i],data2$wtr[t])-data2$QoutInt[t]/data2$epiVol[t]),nrow=2,byrow=T) # parameters estimates are the same as previous time step
    ...
    z$C[,,t,i]<-matrix(c(data2$kCO2[t],1,-data2$GPP[t],0,0,0,0,0,data2$GPP[t],1),nrow=2,byrow=TRUE)
    z$ut[,,t,i] <- matrix(c(data2$DICeq[t]*data2$epiVol[t]/data2$thermo.depth[t],
                            data2$dicIn[t],(1-GPPrespired),
                            exudeTotal,data2$docIn[t]),ncol=1)
...
ylim=range(Y[3,1,,]/data2$epiVol*12)
plot(DOCout/data2$epiVol*12,type='l',ylim=ylim,ylab='DOC mg/L')
for(i in 1:nEn){
  lines(Y[3,1,,i]/data2$epiVol*12,col='gray',ylab='')
}
...
windows()
DOCout<-apply(Y[3,1,,],MARGIN = 1,FUN=mean)
ylim=range(c(Y[3,1,,],z$y[2,1,,1]),na.rm=T)
...
windows()
plot(data2$docIn[spinUpLength:nStep],type='l',ylab='')
par(new=T)
plot(apply(Y[1,1,spinUpLength:length(data2$NEP),],MARGIN = 1,FUN=mean),type='o',col='red',ylab='',yaxt='n')
axis(4)
...
windows()
plot(sqrt((apply(Y[3,1,spinUpLength:nStep,],MARGIN = 1,FUN=mean)/data2$epiVol[spinUpLength:nStep]*12-z$y[2,1,spinUpLength:nStep,1]/data2$epiVol[spinUpLength:nStep]*12)^2),
     pch=16,ylab='DOC RMSE',type='o',xlab='time')

# mean RMSE for states 
sqrt(mean((apply(Y[2,1,spinUpLength:nStep,],MARGIN = 1,FUN=mean)/data2$epiVol[spinUpLength:nStep]*12-z$y[1,1,spinUpLength:nStep,1]/data2$epiVol[spinUpLength:nStep]*12)^2,na.rm=T))
sqrt(mean((apply(Y[3,1,spinUpLength:nStep,],MARGIN = 1,FUN=mean)/data2$epiVol[spinUpLength:nStep]*12-z$y[2,1,spinUpLength:nStep,1]/data2$epiVol[spinUpLength:nStep]*12)^2,na.rm=T))
...
DOCout<-apply(Y[3,1,,],MARGIN = 1,FUN=mean)
ylim=range(c(Y[3,1,,],z$y[2,1,,1]+docPoolSD,z$y[2,1,,1]-docPoolSD),na.rm=T)
...
ylim=range(c(Y[2,1,,],z$y[1,1,,1]+dicPoolSD,z$y[1,1,,1]-dicPoolSD),na.rm=T)
...
for(i in 1:nEn){
  lines(Y[2,1,(spinUpLength+1):nStep,i]~as.POSIXct(data2$datetime[(spinUpLength+1):nStep]),
        col='gray',ylab='',lwd=lwd)
}
...
ylim=range(c(Y[3,1,,]/data2$epiVol*12,z$y[2,1,,1]/data2$epiVol*12+docPoolSD/data2$epiVol*12,z$y[2,1,,1]/data2$epiVol*12-docPoolSD/data2$epiVol*12),na.rm=T)
plot(DOCout[spinUpLength+1:nStep]~as.POSIXct(data2$datetime[spinUpLength+1:nStep]),type='l',ylim=ylim,lwd=lwd,cex.axis=1.5,
...
ylim=range(c(Y[2,1,,]/data2$epiVol*12,z$y[1,1,,1]/data2$epiVol*12+dicPoolSD/data2$epiVol*12,z$y[1,1,,1]/data2$epiVol*12-dicPoolSD/data2$epiVol*12),na.rm=T)
plot(DICout[spinUpLength+1:nStep]~as.POSIXct(data2$datetime[spinUpLength+1:nStep]),type='l',ylim=ylim,lwd=lwd,cex.axis=1.5,
     ylab=expression(CO[2]~(mg~C~L^-1)),xlab='',cex.lab=cex)
for(i in 1:nEn){
  lines(Y[2,1,(spinUpLength+1):nStep,i]/data2$epiVol*12~as.POSIXct(data2$datetime[(spinUpLength+1):nStep]),
        col='gray',ylab='',lwd=lwd)
}
lines(DICout[spinUpLength+1:nStep]~as.POSIXct(data2$datetime[spinUpLength+1:nStep]),ylab='',lwd=lwd)
par(new=T)
