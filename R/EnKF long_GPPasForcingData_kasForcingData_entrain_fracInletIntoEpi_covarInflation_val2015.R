...
hist(fracInVec)
  ...
  print(sqrt(mean((apply(YY[4,1,,],MARGIN = 1,FUN=mean)/data2$epiVol[(nStep14+1):nStep]*12-z$y[1,1,(nStep14+1):nStep,1]/data2$epiVol[(nStep14+1):nStep]*12)^2,na.rm=T)))
  print(sqrt(mean((apply(YY[5,1,,],MARGIN = 1,FUN=mean)/data2$epiVol[(nStep14+1):nStep]*12-z$y[2,1,(nStep14+1):nStep,1]/data2$epiVol[(nStep14+1):nStep]*12)^2,na.rm=T)))
  
  # r2 
  cor(apply(Y[4,1,!is.na(z$y[1,1,,1])&data2$timeStep>nStep14,],MARGIN = 1,FUN=mean)/data2$epiVol[!is.na(z$y[1,1,,1])&data2$timeStep>nStep14]*12,
      z$y[1,1,!is.na(z$y[1,1,,1])&data2$timeStep>nStep14,1]/data2$epiVol[!is.na(z$y[1,1,,1])&data2$timeStep>nStep14]*12)^2
  cor(apply(Y[5,1,!is.na(z$y[2,1,,1])&data2$timeStep>nStep14,],MARGIN = 1,FUN=mean)/data2$epiVol[!is.na(z$y[2,1,,1])&data2$timeStep>nStep14]*12,
      z$y[2,1,!is.na(z$y[2,1,,1])&data2$timeStep>nStep14,1]/data2$epiVol[!is.na(z$y[2,1,,1])&data2$timeStep>nStep14]*12)^2
  
  #bias 
  # mean((apply(Y[3,1,,],MARGIN = 1,FUN=mean)/data2$epiVol*12-z$y[1,1,,1]/data2$epiVol*12)^2,na.rm = T)
  # mean((apply(Y[4,1,,],MARGIN = 1,FUN=mean)/data2$epiVol*12-z$y[2,1,,1]/data2$epiVol*12)^2,na.rm=T)
  mean(apply(Y[4,1,data2$timeStep>nStep14,],MARGIN = 1,FUN=mean)/data2$epiVol[data2$timeStep>nStep14]*12-z$y[1,1,data2$timeStep>nStep14,1]/data2$epiVol[data2$timeStep>nStep14]*12,na.rm = T)^2
  mean(apply(Y[5,1,data2$timeStep>nStep14,],MARGIN = 1,FUN=mean)/data2$epiVol[data2$timeStep>nStep14]*12-z$y[2,1,data2$timeStep>nStep14,1]/data2$epiVol[data2$timeStep>nStep14]*12,na.rm=T)^2
  

# # mean RMSE for states; mol C 
print(sqrt(mean((apply(Y[4,1,data2$timeStep>nStep14,],MARGIN = 1,FUN=mean)-z$y[1,1,data2$timeStep>nStep14,1])^2,na.rm=T)))
print(sqrt(mean((apply(Y[5,1,data2$timeStep>nStep14,],MARGIN = 1,FUN=mean)-z$y[2,1,data2$timeStep>nStep14,1])^2,na.rm=T)))

# r2; mol C 
cor(apply(Y[4,1,!is.na(z$y[1,1,,1])&data2$timeStep>nStep14,],MARGIN = 1,FUN=mean),z$y[1,1,!is.na(z$y[1,1,,1])&data2$timeStep>nStep14,1])^2
cor(apply(Y[5,1,!is.na(z$y[2,1,,1])&data2$timeStep>nStep14,],MARGIN = 1,FUN=mean),z$y[2,1,!is.na(z$y[2,1,,1])&data2$timeStep>nStep14,1])^2
