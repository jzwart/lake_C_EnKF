...
X<-array(NA,dim =c(6,1,nStep,nEn)) # model estimate matrices X[a,b,c,d]; where a=dic/non_co2/co2/doc_r/doc_l/doc, b=column, c=timestep, and d=ensemble member

#initializing estimate of state, X, with first observation
# drawing from normal distribution for inital pool sizes
X[2,1,1,]<-rnorm(n=nEn,y[1,1,1,],sd=dicPoolSD[1])
X[2,1,1,]<-X[2,1,1,] * (1 - frac_co2(dic = X[2,1,1,1], ph = data2$pH_sensor_corr[1], temp = data2$epiTemp[1],
                                       vol_epi = data2$epiVol[1]))
X[3,1,1,]<-rnorm(n=nEn,y[2,1,1,],sd=co2PoolSD[1])
X[1,1,1,]<-X[2,1,1,] + X[3,1,1,] # DIC is non-co2 + co2
X[4,1,1,]<-rnorm(n=nEn,y[3,1,1,]*(1-fracLabile0),sd=docPoolSD[1]*(1-fracLabile0)) # recalcitrant pool is 90% of initial DOC pool
X[5,1,1,]<-rnorm(n=nEn,y[3,1,1,]*fracLabile0,sd=docPoolSD[1]*fracLabile0) # labile pool is 10% of initial DOC pool
X[6,1,1,]<-X[4,1,1,]+X[5,1,1,] # recalcitrant pool + labile pool

# initial B transition matrix for each ensemble
B<-array(NA,dim=c(6,6,nStep,nEn)) # array of transition matrix B[a,b,c,d];
# where [a,b] is transition matrix DIC & DOCr & DOCl, c=timeStep, and d=ensemble member
# initial parameters of B at timestep 1
for(i in 1:nEn){
  B[,,1,i]<-matrix(c(0,

                     1-data2$QoutInt[1]/data2$epiVol[1]-
