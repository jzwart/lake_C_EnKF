...
hist(fracInVec)
  ...
  for(i in 1:nStep){
    ...
    h[2,4,i]<-ifelse(!is.na(y[2,1,i,1]),1,0) #doc total (we only have data on total DOC pool)
  }
  ...
  Y[4,1,1,]<-z$X[2,1,1,] # DOC total state  
  ...
  ylim=range(Y[4,1,,]/data2$epiVol*12)
  ...
  windows()
  DOCout<-apply(Y[4,1,,],MARGIN = 1,FUN=mean)
  ylim=range(c(Y[4,1,,],z$y[2,1,,1]),na.rm=T)
  plot(DOCout,type='l',ylim=ylim,ylab='DOC mg/L')
  for(i in 1:nEn){
    lines(Y[4,1,,i],col='gray',ylab='')
  }
  lines(DOCout,ylab='')
  par(new=T)
  ...
  windows()
  plot(sqrt((apply(Y[4,1,,],MARGIN = 1,FUN=mean)/data2$epiVol*12-z$y[2,1,,1]/data2$epiVol*12)^2),
       pch=16,ylab='DOC RMSE',type='o',xlab='time')
  # 
  # # mean RMSE for states 
  print(sqrt(mean((apply(Y[3,1,,],MARGIN = 1,FUN=mean)/data2$epiVol*12-z$y[1,1,,1]/data2$epiVol*12)^2,na.rm=T)))
  print(sqrt(mean((apply(Y[4,1,,],MARGIN = 1,FUN=mean)/data2$epiVol*12-z$y[2,1,,1]/data2$epiVol*12)^2,na.rm=T)))
  
  # r2 
  cor(apply(Y[3,1,!is.na(z$y[1,1,,1]),],MARGIN = 1,FUN=mean)/data2$epiVol[!is.na(z$y[1,1,,1])]*12,z$y[1,1,!is.na(z$y[1,1,,1]),1]/data2$epiVol[!is.na(z$y[1,1,,1])]*12)^2
  cor(apply(Y[4,1,!is.na(z$y[2,1,,1]),],MARGIN = 1,FUN=mean)/data2$epiVol[!is.na(z$y[2,1,,1])]*12,z$y[2,1,!is.na(z$y[2,1,,1]),1]/data2$epiVol[!is.na(z$y[2,1,,1])]*12)^2
  ...
  mean(apply(Y[3,1,,],MARGIN = 1,FUN=mean)/data2$epiVol*12-z$y[1,1,,1]/data2$epiVol*12,na.rm = T)^2
  mean(apply(Y[4,1,,],MARGIN = 1,FUN=mean)/data2$epiVol*12-z$y[2,1,,1]/data2$epiVol*12,na.rm=T)^2
  

# # mean RMSE for states; mol C 
print(sqrt(mean((apply(Y[3,1,,],MARGIN = 1,FUN=mean)-z$y[1,1,,1])^2,na.rm=T)))
print(sqrt(mean((apply(Y[4,1,,],MARGIN = 1,FUN=mean)-z$y[2,1,,1])^2,na.rm=T)))

# r2; mol C 
cor(apply(Y[3,1,!is.na(z$y[1,1,,1]),],MARGIN = 1,FUN=mean),z$y[1,1,!is.na(z$y[1,1,,1]),1])^2
cor(apply(Y[4,1,!is.na(z$y[2,1,,1]),],MARGIN = 1,FUN=mean),z$y[2,1,!is.na(z$y[2,1,,1]),1])^2
