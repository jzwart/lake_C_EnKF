twoDOCpools_noMM_Temp<-function(obs,freq,reps,rVec_slow,rVec_fast,fracVec,fracInVec,covar_inflat_vec,n){
  docOut<-c()
  docOutBias<-c()
  docOutPrecision<-c()
  dicOut<-c()
  ...
  for(m in 1:n){
    ...
    fracVec<-fracVec
    fracInVec<-fracInVec
    covar_inflat_vec<-covar_inflat_vec
    ...
    Y[5,1,1,]<-covar_inflat_vec # covariance inflation factor 
    ...
    DOCcv<-mean(apply(Y[9,1,,],MARGIN = 1,FUN=sd)/apply(Y[9,1,,],MARGIN = 1,FUN=mean))
    ...
    DICcv<-mean(apply(Y[6,1,,],MARGIN = 1,FUN=sd)/apply(Y[6,1,,],MARGIN = 1,FUN=mean))
...
twoDOCpools_noMM_noTemp<-function(obs,freq,reps,rVec_slow,rVec_fast,fracVec,fracInVec,covar_inflat_vec,n){
  docOut<-c()
  docOutBias<-c()
  docOutPrecision<-c()
  dicOut<-c()
...
twoDOCpools_MM_noTemp<-function(obs,freq,reps,rVec_slow,rVec_fast,fracVec,fracInVec,covar_inflat_vec,n){
  docOut<-c()
  docOutBias<-c()
  docOutPrecision<-c()
  dicOut<-c()
...
twoDOCpools_MM_Temp<-function(obs,freq,reps,rVec_slow,rVec_fast,fracVec,fracInVec,covar_inflat_vec,n){
  docOut<-c()
  docOutBias<-c()
  docOutPrecision<-c()
  dicOut<-c()
...
oneDOCpool_noMM_Temp<-function(obs,freq,reps,rVec,fracInVec,covar_inflat_vec,n){
  docOut<-c()
  docOutBias<-c()
  docOutPrecision<-c()
  dicOut<-c()
  ...
  mdfobsDOCoutBias<-rbind(mdfobsDOCoutBias,mdfobsDOCbias)
  mdfobsDOCoutPrecision<-rbind(mdfobsDOCoutPrecision,mdfobsDOCprecision)
  DOCcv<-mean(apply(Y[5,1,,],MARGIN = 1,FUN=sd)/apply(Y[5,1,,],MARGIN = 1,FUN=mean))
  mdfDOCcv<-rbind(mdfDOCcv,DOCcv)
...
oneDOCpool_noMM_noTemp<-function(obs,freq,reps,rVec,fracInVec,covar_inflat_vec,n){
  docOut<-c()
  docOutBias<-c()
  docOutPrecision<-c()
  dicOut<-c()
...
oneDOCpool_MM_noTemp<-function(obs,freq,reps,rVec,fracInVec,covar_inflat_vec,n){
  docOut<-c()
  docOutBias<-c()
  docOutPrecision<-c()
  dicOut<-c()
...
oneDOCpool_MM_Temp<-function(obs,freq,reps,rVec,fracInVec,covar_inflat_vec,n){
  docOut<-c()
  docOutBias<-c()
  docOutPrecision<-c()
  dicOut<-c()
...
startingValues<-function(){
  ...
  parGuess <- c(0.004,0.3,0.3,0.1,1) #r20; units: day-1; fraction labile of loaded DOC; fraction inlet that goes into epi; turnover rate parameters set constant frac labile estimated 
  min<-c(0.001,0.06,0.005,0.3,-30)
  max<-c(0.001,0.06,0.5,0.5,30)
  ...
  return(list('rVec'=rVec,'rVec_slow'=rVec_slow,'rVec_fast'=rVec_fast,'fracVec'=fracVec,'fracInVec'=fracInVec,'covar_inflat_vec'=covar_inflat_vec))
}
