twoDOCpools_noMM_Temp<-function(obs,freq,reps,rVec_slow,rVec_fast,fracVec,n){
  docOut<-c()
  docOutBias<-c()
  docOutPrecision<-c()
  dicOut<-c()
  ...
  for(m in 1:n){
    ...
    # parGuess <- c(0.002,0.08,0.1) #r20; units: day-1; fraction labile of loaded DOC 
    # min<-c(0.001,0.06,0.05)
    ...
    for(t in 1:nStep){
      curdic<-rnorm(n=reps,mean=true[4,1,t,1]/data2$epiVol[t],sd = dicSD[data2$datetime=='2014-07-30']*obs)
      curdoc<-rnorm(n=reps,mean=true[7,1,t,1]/data2$epiVol[t],sd = docSD[data2$datetime=='2014-07-30']*obs)
    ...
    for(i in 1:nEn){
      B[,,1,i]<-matrix(c(1-data2$QoutInt[1]/data2$epiVol[1]-data2$kCO2[1]/data2$thermo.depth[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1],
                         ...
                         0,1-rFunc(rVec[i],data2$wtr[1])-data2$QoutInt[1]/data2$epiVol[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1],0,0,
                         0,0,1-rFunc(rVec_fast[i],data2$wtr[1])-data2$QoutInt[1]/data2$epiVol[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1],0,
                         0,1-rFunc(rVec[i],data2$wtr[1])-data2$QoutInt[1]/data2$epiVol[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1],
                         1-rFunc(rVec_fast[i],data2$wtr[1])-data2$QoutInt[1]/data2$epiVol[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1],0),
                       nrow=4,byrow=T)
    }
    ...
    for(t in 2:nStep){
      for(i in 1:nEn){
        ...
        z$B[,,t,i]<-matrix(c(1-data2$QoutInt[t]/data2$epiVol[t]-data2$kCO2[t]/data2$thermo.depth[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t], # zmix is fraction of pool available for exchange with atm; parameters estimates are the same as previous time step
                             ...
                             0,1-rFunc(z$pars[1,1,t,i],data2$wtr[t])-data2$QoutInt[t]/data2$epiVol[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t],0,0,
                             0,0,1-rFunc(z$pars[2,1,t,i],data2$wtr[t])-data2$QoutInt[t]/data2$epiVol[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t],0,
                             0,1-rFunc(z$pars[1,1,t,i],data2$wtr[t])-data2$QoutInt[t]/data2$epiVol[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t],
                             1-rFunc(z$pars[2,1,t,i],data2$wtr[t])-data2$QoutInt[t]/data2$epiVol[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t],0),
                           nrow=4,byrow=T)
    ...
    TrueDOC<-true[7,1,,1]/data2$epiVol*12 
    DOCrmse<-sqrt(mean((DOCout-TrueDOC)^2))
    DOCbias<-mean((DOCout-TrueDOC)^2,na.rm=T)
    ...
    obsDOCBias<-mean((obsDOC-TrueDOC)^2,na.rm = T)
    ...
    mdfobsDOCbias<-mean((DOCout-obsDOC)^2,na.rm = T)
    ...
    DOCcv<-mean(apply(Y[7,1,,],MARGIN = 1,FUN=sd)/apply(Y[7,1,,],MARGIN = 1,FUN=mean))
    mdfDOCcv<-rbind(mdfDOCcv,DOCcv)
    
    DICout<-apply(Y[4,1,,],MARGIN = 1,FUN=mean)
    DICout<-DICout/data2$epiVol*12
    TrueDIC<-true[4,1,,1]/data2$epiVol*12 
    DICrmse<-sqrt(mean((DICout-TrueDIC)^2))
    DICbias<-mean((DICout-TrueDIC)^2,na.rm=T)
    ...
    obsDICBias<-mean((obsDIC-TrueDIC)^2,na.rm = T)
    ...
    mdfobsDICbias<-mean((DICout-obsDIC)^2,na.rm = T)
    ...
    DICcv<-mean(apply(Y[4,1,,],MARGIN = 1,FUN=sd)/apply(Y[4,1,,],MARGIN = 1,FUN=mean))
...
twoDOCpools_noMM_noTemp<-function(obs,freq,reps,rVec_slow,rVec_fast,fracVec,n){
  docOut<-c()
  docOutBias<-c()
  docOutPrecision<-c()
  dicOut<-c()
  ...
  for(m in 1:n){
    ...
    for(i in 1:nEn){
      B[,,1,i]<-matrix(c(1-data2$QoutInt[1]/data2$epiVol[1]-data2$kCO2[1]/data2$thermo.depth[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1],
                         ...
                         0,1-rVec[i]-data2$QoutInt[1]/data2$epiVol[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1],0,0,
                         0,0,1-rVec_fast[i]-data2$QoutInt[1]/data2$epiVol[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1],0,
                         0,1-rVec[i]-data2$QoutInt[1]/data2$epiVol[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1],
                         1-rVec_fast[i]-data2$QoutInt[1]/data2$epiVol[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1],0),
                       nrow=4,byrow=T)
    }
    ...
    for(t in 2:nStep){
      for(i in 1:nEn){
        ...
        z$B[,,t,i]<-matrix(c(1-data2$QoutInt[t]/data2$epiVol[t]-data2$kCO2[t]/data2$thermo.depth[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t], # zmix is fraction of pool available for exchange with atm; parameters estimates are the same as previous time step
                             ...
                             0,1-z$pars[1,1,t,i]-data2$QoutInt[t]/data2$epiVol[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t],0,0,
                             0,0,1-z$pars[2,1,t,i]-data2$QoutInt[t]/data2$epiVol[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t],0,
                             0,1-z$pars[1,1,t,i]-data2$QoutInt[t]/data2$epiVol[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t],
                             1-z$pars[2,1,t,i]-data2$QoutInt[t]/data2$epiVol[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t],0),
                           nrow=4,byrow=T)
        z$y[,,t,i]<-z$y[,,t,i] # observation of states stay the same 
        # parameters are of previous timestep for matrix C, parameters of covariates [2x6]
        z$C[,,t,i]<-matrix(c(data2$kCO2[t],1,-data2$ma_gpp[t],0,0,0,data2$hypo_dicInt[t],0,0,0,data2$ma_gpp[t],0,(1-z$pars[3,1,t,i]),data2$hypo_docInt[t]*(1-fracLabile0),
...
twoDOCpools_MM_noTemp<-function(obs,freq,reps,rVec_slow,rVec_fast,fracVec,n){
  docOut<-c()
  docOutBias<-c()
  docOutPrecision<-c()
  dicOut<-c()
  ...
  for(m in 1:n){
    ...
    for(i in 1:nEn){
      B[,,1,i]<-matrix(c(1-data2$QoutInt[1]/data2$epiVol[1]-data2$kCO2[1]/data2$thermo.depth[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1],
                         ...
                         0,1-mm(rVec[i],docVec[1],halfSat,data2$epiVol[1])-data2$QoutInt[1]/data2$epiVol[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1],0,0,
                         0,0,1-mm(rVec_fast[i],docVec[1],halfSat,data2$epiVol[1])-data2$QoutInt[1]/data2$epiVol[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1],0,
                         0,1-mm(rVec[i],docVec[1],halfSat,data2$epiVol[1])-data2$QoutInt[1]/data2$epiVol[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1],
                         1-mm(rVec_fast[i],docVec[1],halfSat,data2$epiVol[1])-data2$QoutInt[1]/data2$epiVol[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1],0),
                       nrow=4,byrow=T)
    }
    ...
    for(t in 2:nStep){
      for(i in 1:nEn){
        ...
        z$X[,,t,i]<-z$B[,,t-1,i]%*%z$X[,,t-1,i] + z$C[,,t-1,i]%*%z$ut[,,t-1,i] # forecasting state variable predictions 
        z$B[,,t,i]<-matrix(c(1-data2$QoutInt[t]/data2$epiVol[t]-data2$kCO2[t]/data2$thermo.depth[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t], # zmix is fraction of pool available for exchange with atm; parameters estimates are the same as previous time step
                             mm(z$pars[1,1,t,i],z$X[4,1,t,i],halfSat,data2$epiVol[1]),mm(z$pars[2,1,t,i],z$X[4,1,t,i],halfSat,data2$epiVol[1]),0,
                             0,1-mm(z$pars[1,1,t,i],z$X[4,1,t,i],halfSat,data2$epiVol[1])-data2$QoutInt[t]/data2$epiVol[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t],0,0,
                             0,0,1-mm(z$pars[2,1,t,i],z$X[4,1,t,i],halfSat,data2$epiVol[1])-data2$QoutInt[t]/data2$epiVol[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t],0,
                             0,1-mm(z$pars[1,1,t,i],z$X[4,1,t,i],halfSat,data2$epiVol[1])-data2$QoutInt[t]/data2$epiVol[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t],
                             1-mm(z$pars[2,1,t,i],z$X[4,1,t,i],halfSat,data2$epiVol[1])-data2$QoutInt[t]/data2$epiVol[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t],0),
                           nrow=4,byrow=T)
        z$y[,,t,i]<-z$y[,,t,i] # observation of states stay the same 
        # parameters are of previous timestep for matrix C, parameters of covariates [2x6]
        z$C[,,t,i]<-matrix(c(data2$kCO2[t],1,-data2$ma_gpp[t],0,0,0,data2$hypo_dicInt[t],0,0,0,data2$ma_gpp[t],0,(1-z$pars[3,1,t,i]),data2$hypo_docInt[t]*(1-fracLabile0),
...
twoDOCpools_MM_Temp<-function(obs,freq,reps,rVec_slow,rVec_fast,fracVec,n){
  docOut<-c()
  docOutBias<-c()
  docOutPrecision<-c()
  dicOut<-c()
...
oneDOCpool_noMM_Temp<-function(obs,freq,reps,rVec,n){
  docOut<-c()
  docOutBias<-c()
  docOutPrecision<-c()
  dicOut<-c()
  ...
  for(m in 1:n){
  # # draws from priors to create ensemble 
  # parGuess <- c(0.005) #r20; units: day-1
  # min<-c(0.001)
  # max<-c(0.02)
  # 
  # rPDF<-abs(rnorm(n=nEn,mean = parGuess[1],sd = (max[1]-min[1])/5)) # max-min / 5 is rule of thumb sd; forcing positive for negative draws 
  ...
  for(i in 1:nEn){
    B[,,1,i]<-matrix(c(1-data2$QoutInt[1]/data2$epiVol[1]-data2$kCO2[1]/data2$thermo.depth[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1],
                       rFunc(rVec[i],data2$wtr[1]),0,1-rFunc(rVec[i],data2$wtr[1])-data2$QoutInt[1]/data2$epiVol[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1]),
                     nrow=2,byrow=T)
  }
  ...
  # what if we alter the process error? i.e. model 
  # data2$kCO2<-data2$kCO2*.1
  ...
  # operator matrix saying 1 if there is observation data available, 0 otherwise 
  h<-array(0,dim=c(2,3,nStep))
  for(i in 1:nStep){
    h[1,2,i]<-ifelse(!is.na(y[1,1,i,1]),1,0) #dic 
    h[2,3,i]<-ifelse(!is.na(y[2,1,i,1]),1,0) #doc
  }
  
  
  P <- array(0,dim=c(1,1,nStep))
  ...
  for(i in 1:nEn){
    ut[,,1,i]<-matrix(c(data2$DICeq[1]*data2$epiVol[1]/data2$thermo.depth[1],
                        data2$dicIn[1],(1-GPPrespired),
                        exudeTotal,data2$docIn[1],data2$entrainVol[1]*data2$entrainEpi[1]),ncol=1)
  }
  
  pars<-array(rep(NA,nEn),dim=c(1,1,nStep,nEn)) # parameters: r20
  pars[1,1,1,]<-rVec
  ...
  # set up Y vector for which we concatonate parameters, states, and observed data 
  Y<-array(NA,c(3,1,nStep,nEn))
  Y[1,1,1,]<-rVec # r20 parameter 
  Y[2,1,1,]<-z$X[1,1,1,] # DIC state  
  Y[3,1,1,]<-z$X[2,1,1,] # DOC state 
  ...
  for(t in 2:nStep){
    for(i in 1:nEn){
      # Forecasting; need to update parameters, 
      z$pars[1,1,t,i]<-Y[1,1,t-1,i] # r20
      z$X[1:2,1,t-1,i]<-Y[2:3,1,t-1,i] # updating state variables from Y 
      
      #Predictions
      z$X[,,t,i]<-z$B[,,t-1,i]%*%z$X[,,t-1,i] + z$C[,,t-1,i]%*%z$ut[,,t-1,i] # forecasting state variable predictions 
      z$B[,,t,i]<-matrix(c(1-data2$QoutInt[t]/data2$epiVol[t]-data2$kCO2[t]/data2$thermo.depth[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t], # zmix is fraction of pool available for exchange with atm 
                           rFunc(z$pars[1,1,t,i],data2$wtr[t]),0,1-rFunc(z$pars[1,1,t,i],data2$wtr[t])-data2$QoutInt[t]/data2$epiVol[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t]),
                         nrow=2,byrow=T) # parameters estimates are the same as previous time step
      z$y[,,t,i]<-z$y[,,t,i] # observation of states stay the same 
      # parameters are of previous timestep for matrix C, parameters of covariates [2x6]
      ...
      z$ut[,,t,i] <- matrix(c(data2$DICeq[t]*data2$epiVol[t]/data2$thermo.depth[t],
                              data2$dicIn[t],(1-GPPrespired),
                              exudeTotal,data2$docIn[t],data2$entrainVol[t]*data2$entrainEpi[t]),ncol=1)
      
      # forecast Y vector 
      Y[1,1,t,i]<-Y[1,1,t-1,i] #r20 parameter same as previous timestep
      Y[2,1,t,i]<-z$X[1,1,t,i] #forecasted DIC state 
      Y[3,1,t,i]<-z$X[2,1,t,i] #forecasted DOC state 
    ...
    if(any(!is.na(z$y[,,t,i]))==TRUE){ # update vector as long as there is one observation of state 
      ...
      #mean of vector Y for all ensembles at time step t 
      YMean<-matrix(apply(Y[,,t,],MARGIN = 1,FUN=mean),nrow=3)
      delta_Y<-Y[,,t,]-matrix(rep(YMean,nEn),nrow=3)# difference in ensemble state and mean of all ensemble states 
      ...
      # assimilating data again if in spin up period
      if(t<spinUpLength){
        #mean of vector Y for all ensembles at time step t
        YMean<-matrix(apply(Y[,,t,],MARGIN = 1,FUN=mean),nrow=3)
        delta_Y<-Y[,,t,]-matrix(rep(YMean,nEn),nrow=3)# difference in ensemble state and mean of all ensemble states
      ...
      # checking for parameter convergence 
      parsMean<-matrix(mean(Y[1,,t,]),nrow=1)
      pars_matrix<-array(NA,dim=c(1,1,nEn))
      for(i in 1:nEn){
        delta_pars<-(Y[1,1,t,i]-parsMean)
        pars_matrix[,,i]<-delta_pars%*%t(delta_pars)
      }
      
      Ptemp<-matrix(0,ncol=1,nrow=1)
      for(i in 1:nEn){
        Ptemp<-Ptemp+pars_matrix[,,i]
      }
  ...
  mdfobsDOCoutBias<-rbind(mdfobsDOCoutBias,mdfobsDOCbias)
  mdfobsDOCoutPrecision<-rbind(mdfobsDOCoutPrecision,mdfobsDOCprecision)
  DOCcv<-mean(apply(Y[3,1,,],MARGIN = 1,FUN=sd)/apply(Y[3,1,,],MARGIN = 1,FUN=mean))
  mdfDOCcv<-rbind(mdfDOCcv,DOCcv)
  ...
  mdfobsDICoutBias<-rbind(mdfobsDICoutBias,mdfobsDICbias)
  mdfobsDICoutPrecision<-rbind(mdfobsDICoutPrecision,mdfobsDICprecision)
  DICcv<-mean(apply(Y[2,1,,],MARGIN = 1,FUN=sd)/apply(Y[2,1,,],MARGIN = 1,FUN=mean))
  mdfDICcv<-rbind(mdfDICcv,DICcv)
...
oneDOCpool_noMM_noTemp<-function(obs,freq,reps,rVec,n){
  docOut<-c()
  docOutBias<-c()
  docOutPrecision<-c()
  dicOut<-c()
  ...
  for(m in 1:n){
    ...
    for(i in 1:nEn){
      B[,,1,i]<-matrix(c(1-data2$QoutInt[1]/data2$epiVol[1]-data2$kCO2[1]/data2$thermo.depth[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1],
                         rVec[i],0,1-rVec[i]-data2$QoutInt[1]/data2$epiVol[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1]),nrow=2,byrow=T)
    }
    ...
    for(t in 2:nStep){
      for(i in 1:nEn){
        ...
        z$X[,,t,i]<-z$B[,,t-1,i]%*%z$X[,,t-1,i] + z$C[,,t-1,i]%*%z$ut[,,t-1,i] # forecasting state variable predictions 
        z$B[,,t,i]<-matrix(c(1-data2$QoutInt[t]/data2$epiVol[t]-data2$kCO2[t]/data2$thermo.depth[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t], # zmix is fraction of pool available for exchange with atm 
                             z$pars[1,1,t,i],0,1-z$pars[1,1,t,i]-data2$QoutInt[t]/data2$epiVol[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t]),
                           nrow=2,byrow=T) # parameters estimates are the same as previous time step
        z$y[,,t,i]<-z$y[,,t,i] # observation of states stay the same 
        # parameters are of previous timestep for matrix C, parameters of covariates [2x6]
        z$C[,,t,i]<-matrix(c(data2$kCO2[t],1,-data2$ma_gpp[t],0,0,data2$hypo_dicInt[t],0,0,0,data2$ma_gpp[t],1,data2$hypo_docInt[t]),nrow=2,byrow=T)
...
oneDOCpool_MM_noTemp<-function(obs,freq,reps,rVec,n){
  docOut<-c()
  docOutBias<-c()
  docOutPrecision<-c()
  dicOut<-c()
  ...
  for(m in 1:n){
    ...
    for(i in 1:nEn){
      B[,,1,i]<-matrix(c(1-data2$QoutInt[1]/data2$epiVol[1]-data2$kCO2[1]/data2$thermo.depth[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1],
                         mm(rVec[i],docVec[1],halfSat,data2$epiVol[1]),0,
                         1-mm(rVec[i],docVec[1],halfSat,data2$epiVol[1])-data2$QoutInt[1]/data2$epiVol[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1]),
                       nrow=2,byrow=T)
    }
    ...
    for(t in 2:nStep){
      for(i in 1:nEn){
        ...
        z$X[,,t,i]<-z$B[,,t-1,i]%*%z$X[,,t-1,i] + z$C[,,t-1,i]%*%z$ut[,,t-1,i] # forecasting state variable predictions 
        z$B[,,t,i]<-matrix(c(1-data2$QoutInt[t]/data2$epiVol[t]-data2$kCO2[t]/data2$thermo.depth[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t], # zmix is fraction of pool available for exchange with atm 
                             mm(z$pars[1,1,t,i],z$X[2,1,t-1,i],halfSat,data2$epiVol[t]),0,
                             1-mm(z$pars[1,1,t,i],z$X[2,1,t-1,i],halfSat,data2$epiVol[t])-data2$QoutInt[t]/data2$epiVol[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t]),
                           nrow=2,byrow=T) # parameters estimates are the same as previous time step
        z$y[,,t,i]<-z$y[,,t,i] # observation of states stay the same 
        # parameters are of previous timestep for matrix C, parameters of covariates [2x6]
        z$C[,,t,i]<-matrix(c(data2$kCO2[t],1,-data2$ma_gpp[t],0,0,data2$hypo_dicInt[t],0,0,0,data2$ma_gpp[t],1,data2$hypo_docInt[t]),nrow=2,byrow=T)
...
oneDOCpool_MM_Temp<-function(obs,freq,reps,rVec,n){
  docOut<-c()
  docOutBias<-c()
  docOutPrecision<-c()
  dicOut<-c()
  ...
  for(m in 1:n){
    ...
    for(i in 1:nEn){
      B[,,1,i]<-matrix(c(1-data2$QoutInt[1]/data2$epiVol[1]-data2$kCO2[1]/data2$thermo.depth[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1],
                         mm(rFunc(rVec[i],data2$wtr[1]),docVec[1],halfSat,data2$epiVol[1]),0,
                         1-mm(rFunc(rVec[i],data2$wtr[1]),docVec[1],halfSat,data2$epiVol[1])-data2$QoutInt[1]/data2$epiVol[1]+data2$entrainVol[1]*data2$entrainHypo[1]/data2$epiVol[1]),
                       nrow=2,byrow=T)
    }
    ...
    for(t in 2:nStep){
      for(i in 1:nEn){
        ...
        z$X[,,t,i]<-z$B[,,t-1,i]%*%z$X[,,t-1,i] + z$C[,,t-1,i]%*%z$ut[,,t-1,i] # forecasting state variable predictions 
        z$B[,,t,i]<-matrix(c(1-data2$QoutInt[t]/data2$epiVol[t]-data2$kCO2[t]/data2$thermo.depth[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t], # zmix is fraction of pool available for exchange with atm 
                             mm(rFunc(z$pars[1,1,t,i],data2$wtr[t]),z$X[2,1,t-1,i],halfSat,data2$epiVol[t]),0,
                             1-mm(rFunc(z$pars[1,1,t,i],data2$wtr[t]),z$X[2,1,t-1,i],halfSat,data2$epiVol[t])-data2$QoutInt[t]/data2$epiVol[t]+data2$entrainVol[t]*data2$entrainHypo[t]/data2$epiVol[t]),
                           nrow=2,byrow=T) # parameters estimates are the same as previous time step
        z$y[,,t,i]<-z$y[,,t,i] # observation of states stay the same 
        # parameters are of previous timestep for matrix C, parameters of covariates [2x6]
        z$C[,,t,i]<-matrix(c(data2$kCO2[t],1,-data2$ma_gpp[t],0,0,data2$hypo_dicInt[t],0,0,0,data2$ma_gpp[t],1,data2$hypo_docInt[t]),nrow=2,byrow=T)
...
startingValues<-function(){
  ...
  rVec_slow<-matrix(rPDF)
  rVec_fast<-matrix(rPDF_fast)
  fracVec<-matrix(fracPDF)
  
  return(list('rVec'=rVec,'rVec_slow'=rVec_slow,'rVec_fast'=rVec_fast,'fracVec'=fracVec))
}
