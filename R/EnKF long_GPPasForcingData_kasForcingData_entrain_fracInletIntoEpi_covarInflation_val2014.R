...
data2<-data2[data2$datetime>as.POSIXct('2015-01-01'),] # only keeping 2015
...
hist(fracInVec)
  ...
  Y15=Y # saving Y parameter for plotting later 
  data15=data2
  z15=z
  # take parameter output from 2015 fit and validate with 2014 data
  ...
  # 
  data2 <- rbind(data2,data15)
  zz <- z
  YY <- Y
  nStep <- length(Y15[1,1,,1])+nStep
  Y <- array(NA,c(5,1,nStep,nEn))
  for(i in 1:length(Y[,1,1,1])){
    for(j in 1:nEn){
      Y[i,1,,j] <- c(YY[i,1,,j],Y15[i,1,,j])
    }
  }
  ...
  for(i in 1:length(z$y[,1,1,1])){
    for(j in 1:nEn){
      z$y[i,1,,j] <- c(zz$y[i,1,,j],z15$y[i,1,,j])
      z$X[i,1,,j] <- c(zz$X[i,1,,j],z15$X[i,1,,j])
    }
  }
  ...
  print(sqrt(mean((apply(Y[4,1,,],MARGIN = 1,FUN=mean)/data2$epiVol*12-z$y[1,1,,1]/data2$epiVol*12)^2,na.rm=T)))
  print(sqrt(mean((apply(Y[5,1,,],MARGIN = 1,FUN=mean)/data2$epiVol*12-z$y[2,1,,1]/data2$epiVol*12)^2,na.rm=T)))
  
  cor(apply(Y[4,1,!is.na(z$y[1,1,,1]),],MARGIN = 1,FUN=mean)/data2$epiVol[!is.na(z$y[1,1,,1])]*12,z$y[1,1,!is.na(z$y[1,1,,1]),1]/data2$epiVol[!is.na(z$y[1,1,,1])]*12)^2
  cor(apply(Y[5,1,!is.na(z$y[2,1,,1]),],MARGIN = 1,FUN=mean)/data2$epiVol[!is.na(z$y[2,1,,1])]*12,z$y[2,1,!is.na(z$y[2,1,,1]),1]/data2$epiVol[!is.na(z$y[2,1,,1])]*12)^2
  
  #bias 
  mean(apply(Y[4,1,,],MARGIN = 1,FUN=mean)/data2$epiVol*12-z$y[1,1,,1]/data2$epiVol*12,na.rm = T)^2
  mean(apply(Y[5,1,,],MARGIN = 1,FUN=mean)/data2$epiVol*12-z$y[2,1,,1]/data2$epiVol*12,na.rm=T)^2
  
  # # mean RMSE for states; mol C 
  print(sqrt(mean((apply(Y[4,1,,],MARGIN = 1,FUN=mean)-z$y[1,1,,1])^2,na.rm=T)))
  print(sqrt(mean((apply(Y[5,1,,],MARGIN = 1,FUN=mean)-z$y[2,1,,1])^2,na.rm=T)))
  
  # r2; mol C 
  cor(apply(Y[4,1,!is.na(z$y[1,1,,1]),],MARGIN = 1,FUN=mean),z$y[1,1,!is.na(z$y[1,1,,1]),1])^2
  cor(apply(Y[5,1,!is.na(z$y[2,1,,1]),],MARGIN = 1,FUN=mean),z$y[2,1,!is.na(z$y[2,1,,1]),1])^2
